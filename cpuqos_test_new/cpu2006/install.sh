...
#  Copyright 1998-2008 by Standard Performance Evaluation Corporation
#
#  Authors:  Christopher Chan-Nui, Cloyce D. Spradling
#
# $Id: install.sh 6002 2008-04-03 04:59:13Z cloyce $
#
...
test_arch_tools() {
    if [ -x "$SPEC/tools/bin/$1/specbzip2" ] &&
       "$SPEC/tools/bin/$1/specbzip2" -h > /dev/null 2>&1 &&
...
for arch in $valid_archs ""; do
  ...
  if [ -z "$arch" ]; then
      ...
      echo ""
      echo "    $SPECTARGET/Docs/tools_build.html"
      echo ""
  ...
  if [ "$SPEC" != "$SPECTARGET" -o ! -d "$SPEC/benchspec" -o ! -d "$SPEC/bin" ]; then
      if [ -n "$SPEC_USE_UNIFIED" -o ! -f "$SPEC/install_archives/release_control" ]; then
        SOC=
        for i in "$SPEC/install_archives/${SUITE}.tar.bz2" "$SPEC/install_archives/${SUITE}.tar.bz2\;1"; do
            if [ -f $i ]; then
                SOC=$i
                break
            fi
        ...
        if [ -z "$SOC" ]; then
            errors="Can't find $SPEC/install_archives/${SUITE}.tar.bz2 file"
        elif [ -z "$errors" ]; then
  ...
  TOOLS=
  for testsuite in cpu2006 mpi2007 ${SUITE}; do
