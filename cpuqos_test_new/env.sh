...
do
    ...
    ssh -p${port} -i ${key} root@${ip} "hostname; rm -rf /root/cpu2006; tar -xf /root/${tools}; apt-get update; apt-get install -y numactl gcc g++ gfortran make; cd /root/cpu2006; ./install.sh; dpkg -l|grep numactl; dpkg -l|grep gcc; dpkg -l|grep g++; dpkg -l|grep gfortran; dpkg -l|grep make"
done
