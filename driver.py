...
CONF.register_opts(cpuqos_opts)
...
CONF.import_opt('enable_cpuqos', 'nova.compute.resource_tracker')
CONF.import_opt('ebs_backend', 'nova.compute.nbs_client')
CONF.import_opt('nbs_mountpoint_prefix', 'nova.compute.nbs_client')
...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def detach_nbs_volume(self, instance_name, mountpoint, host_dev):
        ...
        qos_info = {'iotune_read_bytes': 0,
                    ...
                    'iotune_read_iops': 0,
                    'iotune_write_iops': 0}
        self.update_nbs_qos(instance_name, host_dev, qos_info)
    ...
    def _find_free_dev_and_pcislot(self, instance_name, used_dev):
        ...
        if used_dev is not None:
            if CONF.libvirt_type != 'lxc' and used_dev.get('slot', []) != []:
    ...
    def get_device_for_nbs_volume(self, instance_name, used_dev):
        ...
        if CONF.libvirt_type != 'lxc':
            ...
            real_path = (CONF.nbs_mountpoint_prefix +
                            self._slot_to_disk(free_slot))
        else:
            if free_dev is None:
                return None
            target_dev = '/dev/' + free_dev
            real_path = target_dev
    ...
    def extend_nbs_volume(self, instance_name, device, size):
        ...
        if CONF.libvirt_type == 'lxc':
            LOG.info(_("LXC hypervisor is not needed to extend, "
                       "instance %(inst)s, mountpoint %(mp)s, size %(size)s")
                       % {"inst": instance_name, "mp": device['mountpoint'],
                          "size": size})
            return
    ...
    def update_nbs_qos(self, instance_name, host_dev, qos_info):
        ...
        if ('iotune_read_bytes' in qos_info
            ...
            if rw_bps_value is not None:
                rw_bps = ','.join((host_dev, str(rw_bps_value)))
                params['device_read_bps'] = rw_bps
                params['device_write_bps'] = rw_bps

            rw_iops_value = qos_info.get('iotune_total_iops')
            if rw_iops_value is not None:
                rw_iops = ','.join((host_dev, str(rw_iops_value)))
                params['device_read_iops'] = rw_iops
                params['device_write_iops'] = rw_iops
        else:
            LOG.error(_("qos value of %s not set") % host_dev)

        virt_dom = self._lookup_by_name(instance_name)
        try:
            # FIXME(wangpan): setBlkioParameters of libvirt doesn't support
            #                 VIR_DOMAIN_AFFECT_CONFIG|VIR_DOMAIN_AFFECT_LIVE,
            #                 so I split it to two steps here.
            flag = libvirt.VIR_DOMAIN_AFFECT_CONFIG
            virt_dom.setBlkioParameters(params, flag)
            state = LIBVIRT_POWER_STATE[virt_dom.info()[0]]
            if state == power_state.RUNNING:
        ...
        if CONF.libvirt_type == "lxc":
            ...
            if rescue:
                ...
                if CONF.ebs_backend == 'nbs':
                    for vol in block_device_mapping:
                        cfg = vconfig.LibvirtConfigGuestDisk()
    ...
    def _gen_cpu_qos_paras(self, guest, inst_type):
        ...
        """
        fine_tuning_factors = [float(f) for f in CONF.fine_tuning_factors]
        ...
        if ecus_total < CONF.free_bursting_ecu_limit:
            ecus_per_vcpu += CONF.free_bursting_ecu_num
        ...
        # NOTE(wangpan): libvirt 0.9.* will modify this quota by multiplying
        #                vcpu num, so we needn't do this here.
        #                But libvirt 1.* will NOT do the multiplying operation,
        #                so we need to multiply vcpu num manually.
        guest.cpu_quota = int(float(ecus_per_vcpu) /
                CONF.ecu_ratio * CONF.cpu_period * factor)
        guest.cpu_period = CONF.cpu_period
        ...
        # NOTE(guanqiang): setting of cputune.quota for lxc is different from
        # that of kvm.
        if CONF.libvirt_type == "lxc":
            guest.cpu_quota = guest.cpu_quota * inst_type["vcpus"]
        guest.cpu_shares = ecus_total * CONF.cpu_shares

    def _gen_blkio_info(self, guest, inst_type):
        guest.blk_devices = \
                [dev.get('disk') for dev in CONF.instance_storage_devices]
    ...
    def get_guest_config(self, instance, network_info, image_meta,
        ...
        if CONF.enable_cpuqos:
            guest.cpuset = self._vcpu_pin_set
            self._gen_cpu_qos_paras(guest, inst_type)

        if CONF.use_instance_storage_qos and CONF.libvirt_type != 'lxc':
    ...
    def _get_vcpu_pin_set(self, total_pcpus):
        rule = CONF.vcpu_pin_set.replace(' ', '').replace('\t', '')
    ...
    def get_vcpu_total(self):
        ...
        if CONF.vcpu_pin_set is None or not CONF.enable_cpuqos:
            self._vcpu_total = total_pcpus
            return self._vcpu_total

        if '$' in CONF.vcpu_pin_set:
            ...
            self._vcpu_pin_set = CONF.vcpu_pin_set
        return self._vcpu_total
    ...
    def get_cpu_info(self):
        ...
        if CONF.enable_cpuqos:
            cpu_info['ecu_ratio'] = CONF.ecu_ratio
